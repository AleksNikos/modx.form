...
'h' => 'font-family:Arial;color: #111111;font-weight: 200;line-height: 1.2em;margin: 0 40px 30px;',
'h1' => 'font-size: 36px;',
'h2' => 'font-size: 28px;',
'h3' => 'font-size: 22px;',
  ...
  {block 'logo'}
    <a href="{$site_url}">
      <img style="{$style.logo}"
        src="{$site_url}{'assets_url'|option}/templates/ktm/css/images/logo.png"
        alt="{$site_url}"/>
    </a>
  {/block}
  <!-- body -->
      ...
      <td class="container" style="border:1px solid #f0f0f0;background:#ffffff;width:800px;margin:auto;">
        <div class="content">
          <table style="width:100%;">
            <tr>
              <td>
                <h3 style="{$style.h}{$style.h3}margin-top: 30px;">
                  {block 'title'}
                    miniShop2
                  {/block}
                </h3>
                ...
                {block 'products'}
                  <table style="width:90%;margin:0 auto 30px;">
                    <thead>
                      <tr>
                    ...
                    {foreach $products as $product}
                      <tr>
                        ...
                        <td style="{$style.th}">
                          {if $product.id?}
                            <a href="{$site_url}{$product.id | url}">{$product.name}</a>
                          {/if}
                        </td>
                        <td style="{$style.th}text-align:center;">
                          {$product.count} {'ms2_frontend_count_unit' | lexicon}
                        </td>
                        <td style="{$style.th}text-align:center;">{$product.price} {'ms2_frontend_currency' | lexicon}</td>
                      </tr>
                    {/foreach}
                    <tfoot>
                      <tr>
                        <th style="{$style.th}text-align:right;" colspan="2">Итого</th>
                        <th style="{$style.th}text-align:center;">{$total.cart_count} {'ms2_frontend_count_unit' | lexicon}</th>
                        <th style="{$style.th}">
          ...
          <table style="width:100%; margin:0 auto 30px">
            <tr>
              <td>
                ...
                {block 'infauser'}
                  <table style="width:90%;margin:auto;">
                    ...
                    <tr>
                      <td style="{$style.th}">Варианты доставки</td>
                      <td style="{$style.th}">{$delivery.name}</td>
                    </tr>
                  </table>
                {/block}
              </td>
